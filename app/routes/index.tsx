...
export default function LandingPage() {
	const tools = [
		{
			name: 'TanStack Start',
			description: 'A new React framework focused on routing, caching, and type safety.',
			href: 'https://tanstack.com/start',
		},
		{
			name: 'Convex',
			description: 'A typesafe database that live-updates and automatically invalidates queries.',
	...
	const features = [
		...
		},
		{
			...
			description: 'Hydrating the Query Client in the browser makes SSR take no extra steps.',
			link: '/ssr',
		},
		{
		...
		},
		{
			...
			description: 'Adding queries to loaders enables prefetching and can prevent waterfalls.',
			link: '/loaders',
		},
	];
		...
		<div className="flex flex-col gap-12">
			<div className="gap-6 grid grid-cols-1 lg:grid-cols-2">
				<h1 className="text-6xl font-bold text-balance text-white m-0">
					...
					<span className="text-red-500"> Query</span>, and <span className="text-[#F3B01C]">Convex</span>
				</h1>
				<div className="flex flex-col gap-4 not-prose lg:row-start-2">
					<p className="text-xl font-light text-slate-300">
						TanStack Start is coming. The best way to use Convex with Start is via React Query's excellent
						Start integration. This site is{' '}
						<a href="https://github.com/get-convex/convex-tanstack-start/">written with Start</a> using this
						setup.
					</p>

					<p className="text-xl font-light text-slate-300">
						You can jump straight to the quickstart or read more to learn about using Convex with Start.
						There's a lot more to TanStack Start so also check out the{' '}
				...
				<div className="flex flex-col not-prose text-slate-300 lg:row-start-2">
					<p className="text-sm font-medium">Try out TanStack Start with Convex:</p>
					<p className="text-2xl font-bold mb-8">
						<a href="https://docs.convex.dev/quickstart/tanstack-start">Convex TanStack Quickstart</a>
					</p>
						...
						<a key={tool.href} href={tool.href} target="_blank" rel="noreferrer">
							<Card className="hover:bg-slate-800 transition-colors not-prose">
								<CardHeader className="relative mb-1">
									<CardTitle className="text-3xl font-bold text-center">{tool.name}</CardTitle>
