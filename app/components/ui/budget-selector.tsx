...
export const BUDGET_STEPS = [0, 0.1, 1, 10, 100] as const;
...
export const BudgetSelector = React.forwardRef<HTMLInputElement, BudgetSelectorProps>(
	(
		{ value, defaultValue = 0.1, onChange, name, className, ...props }, //
		ref,
	) => {
		//
		...
		return (
			<div className={`flex flex-row gap-2 ${className}`}>
				<div className="flex flex-col flex-shrink-0">
					...
					<p className="text-sm font-medium tabular-nums">USDC {currentValue.toFixed(2)}</p>
				</div>
